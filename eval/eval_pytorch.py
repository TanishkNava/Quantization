#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""YOLOX PyTorch Quantized Model Evaluation Script (VOC mAP)."""
...
def evaluate_pytorch(cfg: dict, valid_txt: str, data_dir: str, conf_thres: float, nms_thres: float, iou_eval_thres: float):
    model_path = cfg["int8_pytorch"]
    if not Path(model_path).is_file():
        raise FileNotFoundError(f"PyTorch quantized model not found: {model_path}")

    class_names = load_class_names(cfg["classes"])
    num_classes = len(class_names)

    print("\n==============================")
    print("PyTorch Evaluation Settings")
    print("==============================")
...
def main():
    parser = argparse.ArgumentParser("YOLOX PyTorch Quantized Model Evaluation")
    parser.add_argument(
        "--valid_txt",
        required=True,
    ...
    evaluate_pytorch(cfg, args.valid_txt, args.data_dir, args.conf, args.nms, args.iou)
